...
import { BookOpen, Building2, FileText, MapPin, Users, Briefcase, UserCheck, PlusCircle, Trash2, Edit } from 'lucide-react';
...
function SignatureDialog({ onSave, onClear }: { onSave: (dataUrl: string) => void, onClear: () => void }) {
  ...
  return (
    <DialogContent className="max-w-md">
      ...
      <DialogFooter className="sm:justify-between">
        ...
        <DialogClose asChild>
          <Button type="button" onClick={handleSave}>Salvar Assinatura</Button>
        </DialogClose>
      </DialogFooter>
    </DialogContent>
  );
...
export function SafetyForm({ form, onSubmit, isLoading, onNewReport }: SafetyFormProps) {
  ...
  const handleSaveSignature = (dataUrl: string) => {
    if (currentSignatureIndex !== null) {
      form.setValue(`responsiblePersons.${currentSignatureIndex}.signature`, dataUrl, { shouldValidate: true });
    }
  ...
  const handleClearSignature = () => {
     if (currentSignatureIndex !== null) {
      form.setValue(`responsiblePersons.${currentSignatureIndex}.signature`, '', { shouldValidate: true });
    }
  }
  ...
  return (
    <>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className='flex items-center justify-between'>
                ...
                <Button type='button' variant='outline' size='sm' onClick={() => appendResponsible({name: '', role: '', signature: ''})}>
              ...
              {responsibleFields.map((item, index) => (
                <div key={item.id} className='flex items-start gap-2'>
                  ...
                  <div className="flex items-center gap-1 mt-8">
                    <Button type='button' variant='outline' size='sm' onClick={() => handleOpenSignatureDialog(index)}>
                      <Edit className='mr-2 h-4 w-4'/> Assinar
                    </Button>
                    <Button type='button' variant='ghost' size='icon' onClick={() => removeResponsible(index)} disabled={responsibleFields.length <= 1}>
      ...
      <Dialog open={isSignatureDialogOpen} onOpenChange={setIsSignatureDialogOpen}>
        <SignatureDialog onSave={handleSaveSignature} onClear={handleClearSignature}/>
      </Dialog>
    </>
  );
}
