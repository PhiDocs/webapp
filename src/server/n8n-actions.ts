...
export async function notifyN8n(payload: any, webhookUrl?: string, companyId?: string): Promise<N8nSuccess | N8nError> {
  try {
  ...
  try {
    ...
    await ErrorLogRepository.log(error, 'notifyN8n');
    return {
        success: false,
        data: {
