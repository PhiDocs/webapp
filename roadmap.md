# Diretrizer para a IA

- Seja direto e prático na comunicação comigo, sem enrolação ou elogios.
...
- Valide o uso das diretivas `user server` e `use client` para cada implementação nova que fizer, isso está causando muitos erros
...
Ao gerar a Análise de Segurança, não podemos editar esse campo, apenas visualizar, então se houver algum erro não conseguimos atualizar o arquivo.

O arquivo `documento-seguranca.pdf` tem o resultado final do relatório.

Após a seção de `Descrição da atividade` vamos adicionar a opção de adicionar a análise de segurança manualmente e, quando clicar em gerar análise de segurança com IA, os dados serão adicionados ao final dos dados manuais

A pré-visualização e resultado final permanecem inalterados, mas agora podemos trabalhar com os dados sem IA e, ao gerar a análise com IA, podemos alterar se necessário.
